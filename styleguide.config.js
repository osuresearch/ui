...
if (ISOLATED_COMPONENTS) {
    sections = [{
        ...
        description: `Subset of @osuresearch/ui components matching filter: \`${ISOLATED_COMPONENTS}\``,
...
module.exports = {
    ...
    getComponentPathLine: (componentPath) => {
        ...
        if (paths[1] === 'search') {
            if (paths[2] === 'drivers') {
                return `import ${name} from '@osuresearch/ui/dist/search/drivers/${name}'`;
            }

            // Some sort of subcomponent - import line is for the parent
            if (paths[2] === 'components' && paths.length > 3) {
                return `import { ${paths[3]} } from '@osuresearch/ui'`;
            }
        }
        ...
        // `import { Checkbox } from '@osuresearch/ui'` but there's no way to identify
        ...
        return `import { ${name} } from '@osuresearch/ui'`;
    },
    ...
    moduleAliases: {
        ...
        '@osuresearch/ui': path.join(__dirname, './src')
    },
